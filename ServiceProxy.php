...
$authdata->Request = $rawxml;
$authdata->IPAddress = $rawxml->message_header->proxy->client_ip;
...
$response_data->IPAddress = $authdata->IPAddress;
...
function getresponse(response_data $rsp_data){
    ...
    $connectionInfo = array( "Database"=>$dbconfig->db, "UID"=>$dbconfig->user, "PWD"=>$dbconfig->password);
    ...
    $params = array(
        ...
        array($rsp_data->Request, SQLSRV_PARAM_IN),
...
function authenticate(auth_data $authdata){
    ...
    if( $conn === false )
    {
        ...
        $conn_err +=  print_r( sqlsrv_errors(), true);
        $conn_err += "</pre>";
        $auth->auth_err_msg = $conn_err;
    }

    $tsql_callSP = "{call PM.uspGetAuthenticate(?,?)}";
    $params = array(
        array($authdata->Request, SQLSRV_PARAM_IN),
